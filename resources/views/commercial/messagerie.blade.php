...
<head>
    ...
    <style>
        .message-row.unread { background-color: #ffffff; font-weight: 700; color: #000; border-left: 4px solid #0d6efd; }
        .message-row.read { background-color: #f8f9fa; font-weight: 400; color: #6c757d; border-left: 4px solid transparent; }
                ...
                <div class="table-responsive">
                    <table class="table mb-0 align-middle">
                        ...
                        <tbody>
                                ...
                                <tr id="row-{{ $message->id }}" 
                                    ...
                                    <td class="text-center">
                                        <span id="icon-{{ $message->id }}">
                                            @if($message->lu) <i class="fas fa-envelope-open text-muted"></i> @else <i class="fas fa-envelope text-primary"></i> @endif
                                        </span>
                                    </td>
                                    <td>{{ $message->name }}</td>
                                    <td>
                            ...
                            @empty
                                <tr><td colspan="4" class="text-center py-5 text-muted">Aucun message.</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
    ...
    <script>
        function openMessage(id) {
            ...
            if (row.classList.contains('unread')) {
                ...
                icon.innerHTML = '<i class="fas fa-envelope-open text-muted"></i>';
